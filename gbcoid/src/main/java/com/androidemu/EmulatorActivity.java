...
import com.androidemu.gbc.DefaultPreferences;
import com.androidemu.gbc.EmulatorSettings;
import com.androidemu.gbc.MediaScanner;
import com.androidemu.gbc.R;
import com.androidemu.gbc.StateSlotsActivity;
...
public class EmulatorActivity extends Activity implements
		...
		Emulator.OnFrameDrawnListener, GameKeyListener
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		hider = SystemUiHider.getInstance(this, emulatorView,
				SystemUiHider.FLAG_FULLSCREEN | SystemUiHider.FLAG_HIDE_NAVIGATION);
		hider.hide();
		...
		final String[] prefKeys = { "fullScreenMode", "flipScreen", "fastForwardSpeed",
				...
				"soundVolume", "enableTrackball", "trackballSensitivity", "enableSensor",
				"sensorSensitivity", "enableVKeypad", "scalingMode", "enableCheats",
				"orientation", "useInputMethod", "quickLoad", "quickSave", "fastForward",
				"screenshot", };
		...
		startService(new Intent(this, EmulatorService.class)
				.setAction(EmulatorService.ACTION_FOREGROUND).putExtra("target",
						getClass().getName()));
	}
	...
	{
		if (new File(fileName).exists()) emulator.loadState(fileName);
	}
